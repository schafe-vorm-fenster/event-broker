// next js app router GET api route handler

import { createNextHandler } from "@ts-rest/serverless/next";
import { CalendarUpdateByIdContract } from "./calendar-update-by-id.contract";

const handler = createNextHandler(
  CalendarUpdateByIdContract,
  {
    "import-calendar-by-id": async ({ params }) => {
      console.log("params", params);

      // get calendarId fromparams
      const calendarId: string | undefined = params.calendarId;

      // TODO: maybe not needed, move one folder up
      if (!calendarId || calendarId.length === 0) {
        return {
          status: 400,
          body: {
            status: 400,
            error: "Bad Request",
            message: "Calendar ID is required.",
          },
        };
      }

      // TODO: mock empty calendar
      if (
        calendarId ===
        "c_46994e65f2a4d424f41dcd72cbbdf9c8d01cedb2d4ca4eb850d24b6f5de32289@group.calendar.google.com"
      ) {
        return {
          status: 200,
          body: {
            status: 204,
            results: 0,
            timestamp: "2024-12-31T23:59:59.999Z",
          },
        };
      }

      // TODO: mock not found response
      if (calendarId === "unknown-calendar-id") {
        return {
          status: 404,
          body: {
            status: 404,
            error: "Not Found",
            message: "Calendar not found.",
          },
        };
      }

      // TODO: mock not modified response
      if (
        calendarId ===
        "0fc2sup8usvqp7520n0krkn0ipdmmf0o@import.calendar.google.com"
      ) {
        return {
          status: 304,
          body: {
            status: 304,
            results: 0,
            timestamp: "2024-12-31T23:59:59.999Z",
          },
        };
      }

      // TODO: mock successful response
      if (
        calendarId ===
        "fs5r0j9thgru6jto0d3pnnemo49pd36l@import.calendar.google.com"
      ) {
        return {
          status: 201,
          body: {
            status: 201,
            calendarId: calendarId,
            results: 89,
            timestamp: "2024-12-31T23:59:59.999Z",
          },
        };
      }

      return {
        status: 500,
        body: {
          status: 500,
          error: "Internal Server Error",
          message: "An error occurred.",
        },
      };
    },
  },
  {
    handlerType: "app-router",
  }
);

export { handler as GET };
